...
float* forward(Transformer* transformer, int32_t token, int32_t pos, uint32_t flags)
{
    ...
    char* content_row = (char*)w->token_embedding_table_ + token * dim * (size_t)w->dbits_ / 8;
    if(w->dbits_ == 4) {
    ...
    for(int32_t l = 0; l < p->n_layers_; ++l) {
        ...
        for(int32_t e = 0; e < (p->n_experts_ac_ ? p->n_experts_ac_ : 1); ++e) {
            ...
            matmul(s->hb_, s->xb_, (char*)w->w1_[l] + moe_experts[e] * esize, nullptr, dim, hidden_dim, dotprod);
            matmul(s->hb2_, s->xb_, (char*)w->w3_[l] + moe_experts[e] * esize, nullptr, dim, hidden_dim, dotprod);
            ...
            matmul(s->xb2_, s->hb_, (char*)w->w2_[l] + moe_experts[e] * esize, nullptr, hidden_dim, dim, dotprod);
